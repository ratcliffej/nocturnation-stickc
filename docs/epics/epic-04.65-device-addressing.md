...
- Each device class needs its own naming convention (PixMob uses `"all-pixmobs"` and `"group-1..5"`; ESP-NOW Lumes use `"esp-now-broadcast"`; future Tildagon class would invent its own); no unified addressing across classes.
- The Director can broadcast to all Lumes over ESP-NOW (`"esp-now-broadcast"`) but cannot address a subset of Lumes. The Lume-side `PixMobIrBinding` `group` property is per-Lume-static config, not Director-driven.
...
Laptop-driven, same as Epic 4.6. Native test envs catch routing-matrix regressions; hardware verification on Plus2 + S3 with multiple Lumes in different groups confirms the filter logic works end-to-end.
...
- **Director encode path**: `EspNowBroadcastDriver::send` parses the structured target name and writes both `target_class` and `target_group` into the `LightCommandPayload`. The driver-level `device->group_id` carry-through stays (used by PixMob IR driver for its own protocol group byte) but `render_fx` callers now use the structured form.
- **Lume filter**: `LumeMode::on_recv` (or the per-binding fan-out wrapper) compares the inbound `(target_class, target_group)` against each active binding's `class()` AND the device-wide `slv_group` setting. Calls `on_light_command` only where `target_class == 0x00 OR target_class == binding.class()` AND `target_group == 0x00 OR target_group == slv_group`. The group axis is **device-wide** (one slv_group per Lume host), distinct from per-binding group properties: those remain in the OutputBinding for output-protocol-specific reasons (PixMobIrBinding's `group` is the PixMob *protocol's* group code on the IR wire, not the NocturNation receive-filter group).
- **slv_group NVS config**: new device-wide setting persisted under NVS key `slv_group`, default `0` (matches anything). Single Config menu item ("Lume Group", 0-255 manual selection) added under the Lume-mode section. Distinct from the existing Test-Mode "Set Group ID" facility which programs PixMob bracelets via `buildSetGroupId`; this one is the local device's own NocturNation identity.
- **PixMob group range**: bump `PixMobIrBinding`'s `group` property max from 5 to 31 (PixMob protocol's native range; current 0-5 cap was a `LumeMode::ir_target_name` artifact). Device registration adds `"group-6"..."group-31"` in `dal::DAL::begin()`. This is a PixMob-protocol-output concern, orthogonal to slv_group.
...
- **Director-side: visualisations choose their target class**. `BeatPulseVisualisation` and `SpectrumBarsVisualisation` (manual fire) update their `render_fx` calls to `"00:00"` (everyone) by default. A future vis that wants to address only screen-class devices uses `"02:00"`.
...
- Multi-class device hosts. A StickC Lume with both `LocalDisplayBinding` AND `PixMobIrBinding` is conceptually *two* classed devices on one chassis; each binding declares its own class and they're addressed independently. No notion of a "host class" - the binding owns the class.
...
- Director-to-Director class addressing. With one Director per deployment today, Director-side routing isn't a concern.
...
- [ ] **(H)** Plus2 Director + two S3 Lumes: Lume A configured `Light` group 1, Lume B configured `Light` group 2. Director fires `"01:01"` → only Lume A fires PixMob IR. Director fires `"01:02"` → only Lume B. Director fires `"00:00"` → both fire.
- [ ] **(H)** Plus2 Director + Plus2 Lume with `LocalDisplayBinding` only (IR binding disabled in config). Director fires `"02:00"` → Lume's display lights up. Director fires `"01:00"` → Lume does nothing (no `Light` binding active).
- [ ] **(H)** Plus2 Director + S3 Lume with both bindings active, configured to different groups (`LocalDisplay` group 1, `PixMobIr` group 7). Director fires `"02:01"` → only the display lights. Director fires `"01:07"` → only the IR fires.
...
The wire-format break is acceptable - the project's only deployment is the bench, so a coordinated re-flash of Director + Lumes is free. No protocol-version bump (still v1) since the change is additive within a single payload type.

- Commit: "Epic 4.65 Block 3: LightCommandPayload gains target_class (8 -> 9 bytes)"

### Block 4: Target-string parser + Director encode path
...
- Commit: "Epic 4.65 Block 4: render_fx parses class:group; Director encodes target_class"

### Block 5: Lume filter + slv_group config

`LumeMode::on_recv`'s `LIGHT_COMMAND` decode path now reads `target_class` and `target_group` from the payload. For each active binding, only call `on_light_command` if `target_class == 0x00 OR target_class == binding.class()` AND `target_group == 0x00 OR target_group == slv_group`. The `slv_group` axis is **device-wide** (one value per Lume host), loaded from NVS in `LumeMode::enter()` via a new `persistence::load_lume_group()` helper.

Block 5 also adds the Config menu item for `slv_group`: a single line under the Lume-mode section showing the current value with the standard A-click / B-click cycle pattern (0-255 manual selection, persisted on change). Distinct from the existing per-PixMobIrBinding `group` property (which lives in the auto-generated Settings overlay and controls PixMob protocol output, not NocturNation receive filtering).

- Commit: "Epic 4.65 Block 5: Lume filter + slv_group config menu"
...
### Block 7: Director visualisation updates
...
### Block 8: Hardware verification + multi-Lume test deployment

Plus2 Director + two S3 Lumes in different groups + selective-binding scenarios from the acceptance criteria. Catch any wire / encoding / filter mistakes the native suite missed.

- Commit: "Epic 4.65 Block 8: multi-Lume hardware verification fixes" (if any)
...
| Optional: second S3 for multi-Lume H verification | External | Confirm before Block 8 | Jason |

## Status Notes

Pickup from Epic 4.6's architecture stream. Epic 4.6's `OutputBinding` contract made plug-in render destinations possible; this Epic gives them a proper addressing scheme so a Director can talk to "the light-class devices in group 7" rather than "everyone on the wire and let the Lumes figure it out".
...
Total estimated effort: **~5-6 hours of coding plus hardware verification**. Each block is mechanical (one enum, one virtual method, one payload byte, one target-string parser, one Lume filter, a numeric range bump, two call-site migrations, one cleanup pass). Comfortably half a day heads-down, a day with the H lane.

## Close-out (2026-05-11)

All eight code blocks shipped per-block-committed across `main`. Block 8 (hardware verification on Plus2 + S3 multi-Lume scenarios) is the only open item; it's bench work that doesn't gate the architecture changes.
...
- `OutputBinding::device_class()` is pure-virtual; `OutputBinding::is_relay()` defaults false. PixMobIrBinding overrides relay→true so it transmits PixMob IR with the inbound `target_group` regardless of Lume membership.
...
- `DAL::render_fx("<hex_class>:<hex_group>", ev)` parses structured targets; legacy names ("local", "all-pixmobs", "group-N") still resolve via the device registry for the Director's own local IR + screen. The redundant `"esp-now-broadcast"` device-name entry was removed in Block 9; the driver still claims the transport name at the driver layer.
...
- `slv_group` is the Lume's NocturNation receive-filter group ID. Persisted under NVS key `slv_group` (default 0 = match anything); operator sets via Config > ESP-NOW > Group.
- Lume filter: local bindings fire on `(target_class == 0 OR matches) AND (target_group == 0 OR matches slv_group)`. Relay bindings bypass the slv_group axis and read `OutputBindingContext::current_target_group()` to thread the inbound group code into their downstream protocol.
- PixMob group range widened from 0..5 (a `LumeMode::ir_target_name` artifact) to 0..31 (the PixMob protocol's native 5-bit field). `DAL::begin()` registers `group-1`..`group-31`. `kMaxDevices` bumped 32 → 48.
- Director vis (`BeatPulseVisualisation`, `SpectrumBarsVisualisation`) and Test Mode all use `"00:00"` now (broadcast everyone). Director's local IR continues to use `"all-pixmobs"` / `"group-N"` (local PixMob driver, not ESP-NOW).

**Deferred:**
- **(H)** lane: bench verification of multi-Lume scenarios in Block 8 (acceptance criteria 5-8 in the spec above).
