...
- [ ] Audio analysis (FFT beat detection on the StickC's mic) is delivered as `BeatDetected`/`BPMUpdated` events through the DAL's `subscribe_events` API, not via a separate event-source subsystem
