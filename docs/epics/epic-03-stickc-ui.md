...
Implement the boot countdown, mode-selection menu, Test Mode, and Config tree as specified in §8.1-8.6 of the architecture spec. After this Epic, the StickC behaves like a proper Nocturnation device - boots into Lume Mode by default, lets the operator choose other modes via the menu, exposes config for IR/ESP-NOW/Audio settings.
...
- Multi-StickC deployments require re-flashing firmware to switch a device between Director and Lume roles
...
- Boot flow with 5-second countdown to Lume Mode (§8.1)
- Mode-selection menu with Lume / Director / Test / Config options (§8.3)
...
- **Status display polish** - Director already shows mode/BPM/battery/flux meter; need to add an IR fire counter and a "no network yet" placeholder for the network heartbeat.
- **Spec deviations to reconcile** - default boot mode (Lume per spec vs. Director as built); Idle/Off mode (spec lists it separately, implementation collapsed it into Menu).
...
4. **Brand-mark boot splash**: "Noctur**N**ation" with the second N pulsing orange/yellow at ~2 s breathe. Static parts draw once on entry; only the N character cell and seconds counter update each tick (no flicker). Subtitle "Open-source crowd lighting." + countdown line "Director in N s" + hint "press any btn for menu".
...
- **Default boot mode**: Director rather than spec's Lume. Reason: pre-Epic-4 there's no ESP-NOW for Lume to listen on, so Lume is currently a "waiting" placeholder. Revisit when Epic 4 ships.
