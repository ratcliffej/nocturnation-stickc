...
- **Operator UI**: status (current mode, BPM, battery, network state), parameter readouts (FFT level meter, beat indicator), error messages, configuration screens. The current StickC firmware uses the display almost exclusively for this.
...
```
The StickC implements these by drawing on its LCD; the Tildagon implements them on its round screen (and may extend with circle-specific effects); the Atom Lite ignores them (no display); the Core2 might present them as a richer console-style UI.
...
Every Nocturnation node runs the same conceptual state machine, regardless of hardware platform. The UI surface differs (StickC has 2 buttons + screen; Tildagon has 6 buttons + round screen; Atom Lite has 1 button + 1 LED) but the *modes* and the *transitions between them* are common.
### 8.1 Boot flow
...
```plain text
                       ...
                       USB-serial DMX
                              ↓
┌──────────────────────────────────────────────────────┐
│  Master node (StickC near speakers)                  │
│  - Audio analysis                                    │
...
│  A  │  │  B  │  │  C  │  (StickC, Atom Lite, Tildagon)
└─────┘  └─────┘  └─────┘
   ↓ IR     ↓ IR     ↓ IR
 [bracelets in zone A]
                 ...
...
1. **Local beat analysis and IR control for one group via StickC.** Status: ✅ working. Standalone, single-bracelet, no networking. The reference deployment for the Coldplay tribute act.
2. **Group ID setting for PixMob bracelets.** Status: ⏳ unverified - this is the open question that gates the constellation art piece. The `buildSetGroupId` IR command is implemented in `pixmob_protocol.h` but has not been confirmed to work on the user's specific bracelet model (X4 Gen3.1 from KAROL G / Coldplay 2024). If group assignment fails on this model, the constellation art piece needs a redesigned approach (one StickC per bracelet, or all bracelets fire identically with no grouping).
3. **Tildagon app with ESP-NOW slave mode, StickC master transmitting.** Status: ⏳ next-up. Brings Tildagon receiver development forward as the immediate post-tribute priority, ahead of the constellation. Covers the v1 ESP-NOW frame format end-to-end on real hardware and gives EMF app submission lead time.
...
- **Hardware abstraction layer (HAL)** - decouple the firmware from the M5Unified library and from M5StickC-specific GPIO and peripheral assumptions so alternate ESP32 boards (and eventually different microcontroller families) can target the same firmware logic. Surfaced 2026-05-06 when a Wokwi experiment confirmed M5Unified's runtime board detection prevents the firmware from running on any non-StickC ESP32 device. The HAL is a top-level Epic 2 abstraction and must land before any of §10.2's work can claim to be vendor-neutral.
...
For longer-running deployments, USB power banks (10,000 mAh, \~£15) extend StickC and Atom Lite runtime to a full festival day. The bracelets are the easiest part of the rig to power - the original batteries from the events at which they were handed out routinely outlast the rest of the system.
---
## 14. Licensing
