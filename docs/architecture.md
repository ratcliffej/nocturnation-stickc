...
**Vocabulary (Director / Lume):** the upstream node responsible for audio analysis and event broadcast is called the **Director**. Downstream receivers - badges, wristbands, sticks acting as receivers, any future hardware - are called **Lumes**. These terms replace the previous "master / slave" pair throughout this document and the wider documentation set. The codebase still uses the older class names (`MasterMode`, `SlaveMode`, `slv_group`, etc.) as of v0.24; the planned code rename is captured as a future Epic and the older identifiers are noted in this document only where they refer to literal code paths. See §17 for full definitions.
---
## 1. Vision
...
- **Shared professional toolchain.** Once Epic 7 lands DMX support, both PixMob fixtures and NocturNation Directors can be driven from the same QLC+ or grandMA console as DMX-compatible fixtures. A lighting designer treats them as different fixture types in the same show.
- **Adjacent customer bases.** PixMob serves touring artists with stadium-scale budgets; NocturNation serves hackspaces, tribute acts, art installations, and home users. Neither product can comfortably move into the other's territory without becoming a different product.
NocturNation lives in the long tail; PixMob lives at the head. Both ecosystems benefit from the other existing - PixMob's installed hardware base feeds NocturNation's reuse-first model, and NocturNation's open community grows the wider audience-lighting space without competing for the commercial deployments PixMob serves.
---
...
<tr>
<td>**M5StickS3**</td>
<td>Preferred reference platform; Director, IR Tx + Rx node, ESP-NOW peer</td>
...
<td>Solo Director, IR transmitter node</td>
...
<td>0x00</td>
<td>HEARTBEAT</td>
<td>9 bytes: `tick: u32` (monotonic Director clock tick for ASR envelope alignment) • `days_since_2026: u16`  • `centiseconds_today: u24` (both little-endian; for Tier 3 wall-clock anchor + replay protection). Sent at 1 Hz, suppressed when other traffic implicitly proves the Director is alive. Tier 0/1/2 Lumes consume only the tick field; Tier 3 Lumes additionally use the date fields to validate signed frames' freshness against a persisted high-water mark. A single received heartbeat is sufficient to anchor a Tier 3 Lume's replay-protection clock. Replaces the v0.26-era HEARTBEAT (empty), CLOCK_SYNC (0x04), and TIME_SYNC (0x05) which are now folded in. See §4.4 reliability strategy.</td>
</tr>
<tr>
<td>0x03</td>
<td>LIGHT_COMMAND</td>
...
<tr>
<td>0x01, 0x02, 0x04-0xFE</td>
<td>RESERVED</td>
<td>Available for future Director-to-Lume commands (e.g. screen text commands, audio metadata, vendor-specific extensions). Receivers that don't recognise the type byte silently discard the frame - forward-compatible.</td>
</tr>
<tr>
<td>0xFF</td>
<td>EXTENSION</td>
<td>Reserved for v2 protocol extensions; ignored by v1 receivers.</td>
</tr>
</table>
### Why two messages, not one
The minimalism principle has a natural floor: HEARTBEAT and LIGHT_COMMAND are both doing real work, and neither can be folded into the other without losing something the design needs.
...
- **CLOCK_SYNC (0x04)** - folded into HEARTBEAT's tick field. A 1 Hz heartbeat with a 32-bit tick is sufficient to anchor every Lume's clock.
- **TIME_SYNC (0x05)** - folded into HEARTBEAT's date and centiseconds_today fields. Tier 3 receivers get everything they need for replay-attack protection from a single heartbeat.
- **MUSIC_EVENT (0x06)** - was for DROP / BREAKDOWN / BUILD events. The Director already decides what to render on a drop; that decision IS the LIGHT_COMMAND. Lumes don't need the semantic label.
- **SNARE_DETECTED (0x07), HIHAT_DETECTED (0x08), MUSIC_DESCRIPTOR (0x09), SECTION_CHANGE (0x0A)** - all proposed for Epic 4.7. All violated the principle that analysis is Director-internal and the wire carries rendering decisions. Epic 4.7 is now a pure Director-side enhancement: richer audio analysis, richer orchestration, more nuanced LIGHT_COMMAND streams during shows. No protocol changes.
### Reliability strategy
- Receivers track `(source_id, sequence_number)` for last 16 frames; deduplicate.
- The Director sends each event 2-3 times with the same sequence number to spread across airtime gaps.
- Repeaters rebroadcast frames they haven't already seen, with `hop_count + 1`. Cap at 3 hops.
- **Heartbeat at 1 Hz** (revised from the original 4 Hz design during Epic 4 Block 3 hardware verification; the lower duty cycle matters for battery-powered receivers and a \~3 second Director-loss detection window is acceptable for the deployment scenarios in scope). The Director also **skips heartbeat if any other frame went out within the heartbeat period**, so during music with `LIGHT_COMMAND` traffic the alive-signal is implicit and heartbeat traffic drops to roughly zero. Quiet passages in the song where no kick fires for \>1 s legitimately re-trigger the heartbeat so Lumes know the Director is still alive.
...
Receivers (Lumes - Sticks acting as receivers, Tildagon receivers, future bracelets) scan both channels at startup, with channel 11 (show) prioritised over channel 1 (hobby). The scan loops until a heartbeat is heard:
```plain text
...
The scenario worth considering: two commercial shows running concurrently at the same venue (main stage and side stage at a festival, two art installations in adjacent rooms, etc.). Both would naturally want channel 11 / show. Without some discrimination beyond channel and group, every Lume in radio range reacts to both shows simultaneously.
NocturNation's answer is **no universe field in v1**, for several reasons:
- **Existing fields can carry the load.** The `source_id` field (already in every frame header) provides per-Director discrimination. Lumes can be configured to listen only to a specific source. The `target_group` field (in LIGHT_COMMAND payload) provides per-device-collection discrimination. Show A using groups 1-15 and Show B using groups 16-31 is a perfectly serviceable workaround.
...
**Power architecture: two product tiers, not one.** A practical observation that reshapes the mic-less device thinking: there are two distinct power architectures the mic-less device could use, and they serve genuinely different markets rather than one being universally better.
*Tier A: AAA-powered receiver.* 2 × AAA + boost converter to 3.3V. Eliminates the LiPo cell, the TP4056 charging IC, the USB-C connector, and the associated charging infrastructure. Adds a battery holder and a small boost converter (\~£0.30-0.80). Net BOM saving is modest at hobby scale (\~£0.50-2 per unit) but the operational benefits are substantial: shipping classification drops from UN3481 dangerous goods to standard parcel; safety certification (CE/UKCA) is far simpler without LiPo thermal-runaway assessment; insurance liability for the product is lower; the device has no end-of-life capacity-degradation curve (LiPos degrade to \~80% capacity after \~300 cycles, AAAs are replaceable indefinitely). Runtime is also dramatically better - 2×AAA alkaline gives \~20× the energy of a 150mAh LiPo, meaning a single set of batteries lasts a full festival weekend or weeks of casual home use. The trade-off is form factor: 2×AAA in a holder is roughly five times the volume and weight of a 150mAh LiPo, taking the device from "barely noticeable wristband" to "strapped-on remote control" feel.
*Tier B: LiPo-rechargeable wearable.* Small LiPo cell + integrated TP4056 charging + USB-C for both data and charging. Cleaner BOM at commercial volume, smaller and lighter form factor (suitable for wristband / pendant / jewelry-like wearables), familiar phone-style charging UX that consumers expect from premium wearables. The trade-off is hazmat-class shipping, additional certification overhead, finite cycle life, and roughly 1.5-2 hour runtime per charge.
Different deployment contexts naturally fit different tiers:
- **Tier A (AAA) wins for**: single-event wristbands sold as cheap merch (£5-10 retail), festival weekend deployments where 30+ hour runtime matters, static art deployments (constellation art piece, hat-mounted units, hanging pendants where form factor doesn't matter), the Tildagon receiver-only role (already fits the badge-with-replaceable-batteries pattern), mic-less devices distributed at hacker camps. The "rugged" tier.
- **Tier B (LiPo) wins for**: the home-use companion-app device where the user expects to charge it like a phone (AAA in a £30 wristband feels dated), K-pop-style branded artist merch (£20-40 retail, premium feel, rechargeable), compact-and-light wristband form factors where the device needs to be barely-noticeable jewelry, repeated regular use cycles where charging-once-a-week is normal. The "consumer" tier.
Same firmware, same protocol, same effects catalogue. Different physical product positioning. This is a useful insight because it changes how the mic-less device direction is framed: rather than "the mic-less device" being one product, it's potentially two product tiers serving different markets. The architecture commits to *neither* tier specifically and supports both, which means the firmware doesn't need to know whether it's running on AAA or LiPo - power management is a hardware-design concern, not a protocol concern.
A receiver-only class name is currently undecided (PixMob calls theirs "pixels"; NocturNation's equivalent is a brand decision worth taking deliberately rather than reflexively). The class would cover all receiver-only configurations across both Tier A and Tier B - the name describes the *role*, not the form factor or power source.
...
Group ID is a first-class concept in the NocturNation protocol, not a PixMob-IR-specific feature. Every Lume that understands LIGHT_COMMAND has an assigned group ID, persisted across reboots. The semantics:
- **Group 0** - broadcast. All Lumes act on the command. Matches PixMob's protocol semantics for compatibility.
...
For step-by-step developer guidance on adding a new Show — Show base class API, registration, analyser hooks, widget composition, persistence, testing — see [docs/](https://github.com/ratcliffej/nocturnation-stickc/blob/main/docs/developing-shows.md)[developing-shows.md](http://developing-shows.md).
...
**Slave-side: ****`OutputBinding`**** plug-ins.** *(Note: "Slave-side" reflects code identifiers; the operational role is the Lume side.)* An output binding consumes `RenderEvent`s and turns them into hardware action. The contract lives in [`include/output_bindings/output_binding.h`](https://github.com/ratcliffej/nocturnation-stickc/blob/main/include/output_bindings/output_binding.h): same Plugin base as Visualisation, same property-bag and capability machinery. Differences from Visualisation: bindings have no `render_fx` accessor (they *are* the render destination, calling `render_fx` from one would be circular) and their hook surface is `on_light_command` rather than `on_audio_frame`. `SlaveMode` is a thin shell that fans inbound `LIGHT_COMMAND` events out to every registered binding.
Two bindings ship today: `LocalDisplayBinding` (paints the Lume's LCD full-bleed with the broadcast colour and ASR envelope - the "display-as-light" behaviour from Epic 4) and `PixMobIrBinding` (IR + PixMob protocol with a `group` property, 0=broadcast, 1-31=specific group; Epic 4.65 widened the range from the original 0-5 cap which was a `SlaveMode` artifact). Both can run simultaneously; either can be disabled in Config to limit the Lume to one output. The legacy NVS keys (`slv_ir_grp`) migrated one-shot to the per-binding namespace at first boot.
...
- `0x01` **Light** - light-only Lumes (PixMob X4 bracelets via `PixMobIrBinding`; future NocturNation-native light wristbands).
- `0x02` **Screen** - screen-only Lumes (`LocalDisplayBinding` on a StickC Lume).
- `0x03` **MultiLedScreen** - multi-LED + screen Lumes (Tildagon-class; `TildagonLedRingBinding` returns this in Epic 5).
...
<td>A beat source (mic analyser **or** IMU tap-to-beat) + at least one output. Most hosts use the mic analyser; the Tildagon has **no microphone** but runs an **IMU tap-to-beat Director** (Epic 6B, §8.9). The mode-selection menu offers Director on any host with a beat source.</td>
</tr>
<tr>
<td>**Lume**</td>
...
- **Group** *(direct action)* — increments `slv_group` 0..255; the device-wide NocturNation receive-filter group ID applied by Lume Mode upstream of binding fan-out. *(NVS key name **`slv_group`** retained for backwards compatibility with existing deployments; planned for rename in the code-rename Epic.)*
...
<tr>
<td>Btn1 short click</td>
<td>`Confirm`</td>
</tr>
<tr>
<td>Btn1 long press</td>
<td>`Settings`</td>
</tr>
<tr>
<td>Btn1 double-click</td>
<td>`Pause`</td>
</tr>
<tr>
<td>Btn2 short click</td>
<td>`Cycle`</td>
</tr>
<tr>
<td>Btn2 long press</td>
<td>`Picker`</td>
</tr>
</table>
...
`AutonomousMasterMode` *(class identifier retained pending code rename Epic)* runs the active Show full-screen by default but exposes two operator overlays without leaving the mode:
...
The Tildagon runs the same `Show` plug-in framework as the M5 firmware, ported to MicroPython with **identical hook names + capability vocabulary**. A Show is a folder under `apps/nocturnation/shows/<id>/` exposing `make_show()`, auto-discovered at boot. Host-specific bits differ (IMU vs mic input; `ctx.display()` vs `DAL::fire_display_*`; `RgbPulse` vs `RgbPulseEvent`) but the orchestration model is shared.
- **Input**: with no microphone, the Tildagon Director's beat source is the **IMU** — gravity-EMA high-pass onset detection fires `on_tap_detected` (+ `on_beat_detected`, so a beat-driven Show is host-agnostic) and `on_motion_event`; a button-tap fallback covers untuned IMUs. So Director Mode requires *a beat source (mic analyser or IMU tap) + an output* — not necessarily a microphone, superseding the earlier "hosts without a mic cannot enter Director Mode" note.
- **Output**: `ctx.render_fx("<class>:<group>", RgbPulse)` fans to ESP-NOW broadcast + the Director's own perimeter LEDs + LCD, plus a 1 Hz HEARTBEAT beacon and 3× redundancy. The Tildagon Director transmits on **channel 1 only** (Epic 5.5).
- **Radio / WiFi coexistence (platform constraint)**: the Tildagon's WiFi + ESP-NOW share one radio, and an idle unassociated WiFi STA makes the firmware channel-sweep (breaking ESP-NOW). The app launches into an idle state with WiFi up, and only takes the radio (`wifi.stop()`) while a Lume/Director session runs; WiFi is restored on stop/Quit. A deployed badge therefore has no WiFi while lighting — by design.
...
- **How does a Lume indicate "I'm not receiving"?** Idle effect after timeout is the default; should there also be a visual cue (flashing red dot on screen, etc.)?
---
...
PixMob Inc. (n.d.) *PixMob - immersive light experiences* \[Company website\]. Available at: [https://www.pixmob.com/](https://www.pixmob.com/) (Accessed: 14 May 2026). Vendor reference for the commercial PixMob bracelet ecosystem (X4, Aurora, Helios product lines) and named tour deployments. Referenced in §1.1 for market-positioning context. NocturNation's IR encoder reproduces the byte-level PixMob CommandSingleColorExt protocol per the reverse-engineering work cited above (Weidman, 2022; W., J., 2024); the company itself publishes no protocol-level documentation.
...
- **v0.23** (2026-05-10): expanded §4.5 companion-app forward direction with a **"Power architecture: two product tiers, not one"** subsection. Captures the AAA-powered vs LiPo-rechargeable trade-off explicitly, framing them as serving genuinely different markets rather than one being universally better. Tier A (AAA-powered): cheaper BOM at hobby scale, dramatically better runtime (\~20×), no hazmat shipping, simpler certification, indefinite battery replaceability - but \~5× the form factor and weight. Wins for festival weekend wristbands, static art deployments, the Tildagon receiver role, mic-less hacker-camp distribution. Tier B (LiPo-rechargeable): smaller/lighter form factor, USB-C charging UX consumers expect, premium feel - but hazmat shipping, finite cycle life, shorter runtime. Wins for home-use companion-app devices, K-pop-style branded merch, compact jewelry-like wearables. Same firmware/protocol/effects across both - the architecture commits to neither tier specifically. Also added a note that a receiver-only class name is currently undecided (PixMob calls theirs "pixels"; NocturNation's equivalent is a deliberate brand decision pending), and that whichever name lands will describe the *role* across both tiers, not the form factor or power source.
- **v0.24** (2026-05-14): **Vocabulary rename - master → Director, slave → Lume.** The upstream node responsible for audio analysis and event broadcast is now called the **Director**; downstream receivers are called **Lumes**. Renaming applied throughout the spec body except where the original identifier is a literal code symbol (e.g. `MasterMode`, `SlaveMode`, `slv_group`, `AutonomousMasterMode`) that the codebase still uses; those remain as code references with a noted pending rename. The rename pairs with the receiver-only class name decision that was "undecided" in v0.23 - Lume is now the canonical NocturNation equivalent of PixMob's "pixels". New glossary entries in §17 capture both terms with deprecation notes for the older vocabulary. Headline note added at the top of the spec flagging the rename. The vocabulary change sidesteps the master/slave terminology baggage common in modern open-source projects (Git, Python, Redis et al have similar migrations) and pairs naturally with the theatre-adjacent project voice. A planned code-rename Epic will close the gap between the documentation and the codebase; until then, documentation uses Director / Lume and the code uses the older names.
- **v0.25** (2026-05-14): added **§1.1 "Where NocturNation sits in the market"** subsection to the Vision section. Frames four adjacent product categories - manual RF wristbands (TheWristbandCo and similar), PixMob / Xylobands (vertically-integrated arena-scale automated systems), professional DMX-driven rigs (QLC+ / ETC Eos / grandMA driving fixtures), and NocturNation itself - and positions NocturNation as architecturally distinctive because it can operate in *either* autonomous mode (like PixMob, audio-reactive, no console required) *or* DMX-driven mode (like a professional fixture, integrated with lighting consoles). The honest caveat is captured explicitly: NocturNation is not trying to displace PixMob at stadium scale; it lives in the long tail where the architectural priorities are inversely-aligned with PixMob's vendor-controlled model. Added in response to a question about how DMX512 compatibility relates to the existing IR / RF / ESP-NOW carrier choices in §4.1, and the related question of what TheWristbandCo's product actually does. Pre-empts the obvious "why didn't you just use PixMob / RF wristbands / DMX like everyone else?" question that prospective contributors will ask. New references added to §12 covering PixMob and the DMX512 standard.
- **v0.26** (2026-05-14): refined the closing paragraph of §1.1 from "both can coexist solving different problems" to **"complementary, not competitive"**. The earlier framing was too neutral and missed the genuinely interesting strategic point: PixMob's hardware ecosystem is what gives NocturNation its reuse-first deployment model, and the two products can run in the same room at the same time without either disrupting the other. Four specific complementarity vectors captured: hardware reuse (recycled PixMob wristbands as NocturNation Lumes), mixed deployment at the same event (different carriers + channels, no wire conflict), shared professional toolchain (once Epic 7 lands DMX, both fixture types live in the same QLC+ console), and adjacent customer bases (PixMob serves stadium-scale touring artists; NocturNation serves the long tail). Strengthens the project's positioning narrative for prospective contributors, sponsors, and the wider lighting community without softening the architectural distinctions.
- **v0.27** (2026-05-16): **protocol minimalism pass.** Trimmed §4.3 message types from eleven (eight active + reserved) down to two active types (HEARTBEAT + LIGHT_COMMAND) plus a reserved range and the EXTENSION slot. Folded CLOCK_SYNC's tick field and TIME_SYNC's date + centiseconds_today into a new 9-byte HEARTBEAT payload, so a single 1 Hz heartbeat now anchors both the ASR envelope clock (Tier 0+) and the Tier 3 replay-protection wall-clock simultaneously. Removed BEAT_DETECTED (no Lume-side consumer existed), MODE_CHANGE (Lumes don't need the Director's semantic mode), MUSIC_EVENT (the Director encodes DROP / BREAKDOWN decisions as LIGHT_COMMAND), and the four Epic 4.7 message types (SNARE_DETECTED, HIHAT_DETECTED, MUSIC_DESCRIPTOR, SECTION_CHANGE - all violated the wire-carries-only-render-commands principle). Epic 4.7 is now a pure Director-side enhancement: richer analysis, richer orchestration, more nuanced LIGHT_COMMAND streams during shows, no protocol changes. Rationale: a Lume can't make an artistic decision; the wire doesn't need to carry the data that would enable one. No version bump on the wire because the project isn't deployed yet - this is correction of an over-decomposed design, not an upgrade. The new §4.3 "Messages removed from earlier drafts" subsection records each removed type with the reasoning, so future contributors don't reflexively re-add them.
- **v0.29** (2026-05-16): added new "Why two messages, not one" subsection to §4.3, documenting the floor on the minimalism principle. Records that HEARTBEAT and LIGHT_COMMAND occupy genuinely separate roles (ambient presence channel vs render decision channel) that cannot be merged without losing real capability. Crucially captures the rationale that HEARTBEAT lets Lumes lock onto a channel *before* a show starts, supporting the dual-channel scan in §4.5 - silence between songs and pre-show idle would otherwise be indistinguishable from Director loss. The subsection pre-empts a future contributor suggesting "why have a separate HEARTBEAT type, every LIGHT_COMMAND counts as a heartbeat" - that suggestion would over-apply the minimalism principle and break the pre-show channel lock. Also flagged that future additions (screen text, audio metadata, vendor extensions) belong in the reserved 0x01, 0x02, 0x04-0xFE range as new message types when needed, not as fields stuffed into the existing two.
- **v0.30** (2026-05-21): Epic 6B reconciliation. Promoted the **EMF Tildagon badge** from §3.2 Planned to §3.1 Currently supported and replaced the **Lume-only / "cannot be a Director"** framing with **Lume + IMU tap-to-beat Director** - resolving the contradiction with §8.9 (which already described the Tildagon Director). Fixed the matching §8.2 operating-modes cell ("hosts without a mic cannot enter Director Mode" → "a beat source: mic analyser *or* IMU tap") and updated §10.1 item 3 from ⏳ next-up to ✅ working (bench-verified) with Director/Lume vocabulary. Corrected the §3 Tildagon LED count from six to twelve perimeter LEDs for consistency with §6/§7/§8.9. Also brought the local working copy (docs/[architecture.md](http://architecture.md)) up from its v0.22-era base to match this Notion revision - the v0.24 Director/Lume rename, v0.25-v0.26 §1.1 market positioning, and v0.27/v0.29 protocol-minimalism pass had been Notion-only.
