...
MicStickCS3::MicStickCS3()
    : fft_(v_real_, v_imag_, kFftSize, (double)kSampleRate) {}

void MicStickCS3::begin(uint16_t /*sample_rate_hz*/, uint16_t /*fft_size*/) {
    if (running_) return;
    // Sample rate / FFT size are fixed in this backend (see header). Honouring
    // requested overrides is a future refinement.
...
void MicStickCS3::poll() {
    ...
    // record() is blocking (~32 ms at 16 kHz / 512). Same cost the Plus2
    // backend pays; will reduce sharply once the esp-dsp FFT upgrade lands.
